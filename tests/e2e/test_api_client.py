...
class TestAPIClient(E2eBase):
    ...
    def test_list_tasks(self):
        ...
        async def run_test():
            ...
            if tasks:  # If there are tasks, verify structure
                task = tasks[0]
                expected_fields = [
                    "task_id", "task_type", "name", "description", "status",
                    "priority", "created_at", "updated_at", "created_by", "_links"
                ]
            ...
            print(f"   Pagination: limit={pagination['limit']}, has_more={pagination['has_more']}")
        ...
        assert result == expected_data, "Async client result should match direct API response"
    ...
    def test_list_variables(self):
        ...
        async def run_test():
            ...
            if result:  # If there are variables, verify structure
                variable = result[0]
                expected_fields = ["name", "description", "created_at", "updated_at", "created_by"]
                for field in expected_fields:
                    assert field in variable, f"Variable should contain '{field}' field"
                
                # Verify field types
                assert isinstance(variable["name"], str), "Variable name should be a string"
                assert isinstance(variable["created_at"], str), "Created_at should be a string"
                assert isinstance(variable["updated_at"], str), "Updated_at should be a string"
                assert isinstance(variable["created_by"], str), "Created_by should be a string"
    ...
    def test_list_tasks_with_filters(self):
        ...
        async def run_test():
            ...
            pagination = result["pagination"]
            assert pagination["limit"] == 5, "Pagination limit should match requested limit"
