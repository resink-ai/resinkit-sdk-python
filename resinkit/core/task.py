...
from .resinkit_api_client import ResinkitAPIClient
...
class Task:
    ...
    def __init__(self, task_id: str, base_url: Optional[str] = None, api_key: Optional[str] = None):
        """
    ...
    def result_df(self) -> Union[pd.DataFrame, List[pd.DataFrame]]:
        """
        ...
        if not query_results:
            raise ValueError(f"No query results found for task {self.task_id}. All statements were non-query operations.")
        
        # Return single DataFrame if only one result, otherwise return list
