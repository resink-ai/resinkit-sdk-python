...
class TasksManagementUI(param.Parameterized):
    ...
    def _create_task_submit_view(self):
        ...
        header = pn.Row(
            ...
            pn.Row(
                pn.widgets.Button(name="← Back to Tasks", button_type="default", width=150, on_click=self._back_to_list),
                sizing_mode="fixed"
            ),
            sizing_mode="stretch_width"
        )
...
"""
        ...
        submit_btn = pn.widgets.Button(name="Submit Task", button_type="primary", width=150)
    ...
    def _create_task_detail_view(self):
        ...
        header = pn.Row(
            ...
            pn.Row(
                pn.widgets.Button(name="← Back to Tasks", button_type="default", width=150, on_click=self._back_to_list),
                pn.widgets.Button(name="↻ Refresh", button_type="default", width=100, on_click=self._load_task_details),
                sizing_mode="fixed"
            ),
            sizing_mode="stretch_width"
        )
...
"""
            ...
            if task.get('status') == 'COMPLETED':
                ...
                self.task_results.object = {"message": "Results only available for completed tasks"}
...
class ResinkitAPIClient:
    ...
    def submit_yaml_task(self, yaml_config):
        """Submit a new task with YAML configuration."""
        import requests
    ...
    def cancel_task(self, task_id, reason=None, force=False):
        """Cancel a task."""
        import requests
